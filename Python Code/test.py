...
if (level == 'level1'):
    ...
    testdata = pn.read_csv(datapath + 'golden-data.csv',sep=',',names=r_cols,usecols=range(2),na_filter=True,header=None)
elif (level == 'level2'):
    data = pn.read_csv(datapath + 'clean-training-data.csv',sep=',',names=r_cols,usecols=range(2),na_filter=True,header=None)
    testdata = pn.read_csv(datapath + 'clean-golden-data.csv',sep=',',names=r_cols,usecols=range(2),na_filter=True,header=None)
elif (level == 'level3'):
    data = pn.read_csv(datapath + 'fixed-training-data.csv',sep=',',names=r_cols,usecols=range(2),na_filter=True,header=None)
    testdata = pn.read_csv(datapath + 'fixed-golden-data.csv',sep=',',names=r_cols,usecols=range(2),na_filter=True,header=None)

data = data.dropna()
testdata = testdata.dropna()
vectorizer = CountVectorizer()
traincounts = vectorizer.fit_transform(data['tweet'].values)
testcounts = vectorizer.transform(testdata['tweet'].values)
d = {'positive':1, 'negative':0}
targets = testdata['sentiment'].map(d)
totalPredictions = []
...
def benchmark(clf,name) :
    ...
    #Training
        t0 = time()
        classifier = clf
        classifier.fit(traincounts, targets)
        train_time = time() - t0
...
#Saving the results    
with open(path + 'result/' + level + '/' + 'golden-results.pkl', 'wb') as fid:
...
test_time = np.array(score) / np.max(score)

plt.title("Score")
plt.barh(indices, score, .6, label="score", color='r')
plt.yticks(())
...
corrnum.to_excel(excel_writer = path + 'result/' + level + '/' + 'golden-corrcoef.xlsx')
